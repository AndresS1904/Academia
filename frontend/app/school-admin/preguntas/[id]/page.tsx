...
interface OptionForm { id?: string; letra: string; texto: string; imageUrl: string; isCorrect: boolean; }
interface QuestionTag { id: string; name: string; color?: string; }
interface QuestionDetail {
  id: string; enunciado: string; contexto?: string; imageUrl?: string; area: string;
  examType: string; difficulty: string; explicacion?: string; isActive: boolean; isGlobal: boolean;
  topic?: string; subtopic?: string; competence?: string; component?: string;
  gradeLevel?: number; year?: number; sourceType?: string;
  subject?: string; questionType?: string;
  tags: { tag: QuestionTag }[];
  options: { id: string; letra: string; texto: string; imageUrl?: string; isCorrect: boolean; }[];
}

const COMMON_AREAS = ["Lectura Crítica", "Matemáticas", "Ciencias Naturales", "Ciencias Sociales", "Inglés", "Razonamiento Cuantitativo"];
const SOURCE_TYPES = [
  { value: "PROPIA", label: "Propia" }, { value: "ICFES", label: "ICFES" },
  { value: "UDEA", label: "U de A" }, { value: "SABER", label: "Saber" }, { value: "OTRA", label: "Otra" },
];
...
export default function EditPreguntaSchoolPage() {
  ...
  useEffect(() => {
    ...
    api.get<QuestionDetail>(`/questions/${id}`)
      .then(q => {
        ...
        if (COMMON_AREAS.includes(q.area)) { setArea(q.area); setAreaCustom(""); }
        else if (q.area) { setArea("__custom__"); setAreaCustom(q.area); }
        setTopic(q.topic ?? ""); setSubtopic(q.subtopic ?? "");
        setCompetence(q.competence ?? ""); setComponent(q.component ?? "");
        setGradeLevel(q.gradeLevel ? String(q.gradeLevel) : "");
        setYear(q.year ? String(q.year) : "");
        setSourceType(q.sourceType ?? "PROPIA");
        if (q.tags) setSelectedTagIds(q.tags.map(t => t.tag.id));
      ...
      .catch(() => router.replace("/school-admin/preguntas"))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await api.patch(`/questions/${id}`, {
        enunciado: enunciado.trim(), contexto: contexto.trim() || undefined,
        imageUrl: imageUrl.trim() || undefined, area: resolvedArea.trim(),
        examType, difficulty, explicacion: explicacion.trim() || undefined,
        topic: topic.trim() || undefined, subtopic: subtopic.trim() || undefined,
        competence: competence.trim() || undefined, component: component.trim() || undefined,
        gradeLevel: gradeLevel ? parseInt(gradeLevel) : undefined,
        year: year ? parseInt(year) : undefined, sourceType,
  ...
  if (fetching) {
    return (
      <div className="admin-layout">
        <SchoolAdminSidebar />
        <main className="admin-main"><div className="admin-content" style={{ textAlign: "center", padding: 60, color: "#64748b" }}>⏳ Cargando pregunta…</div></main>
      </div>
    );
  }
      ...
      <div className="admin-layout">
        ...
        <main className="admin-main">
          <div className="admin-content">
            <div style={{ background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 14, padding: "28px 32px", textAlign: "center" }}>
              <div style={{ fontSize: "2rem", marginBottom: 12 }}>🔒</div>
              <h2 style={{ fontWeight: 800, color: "#1e293b", marginBottom: 8 }}>Pregunta global</h2>
              <p style={{ color: "#64748b" }}>Esta pregunta es del sistema global y no puede ser editada por tu institución.</p>
              <Link href="/school-admin/preguntas" style={{ display: "inline-block", marginTop: 16, padding: "10px 24px", background: "#004aad", color: "#fff", borderRadius: 10, textDecoration: "none", fontWeight: 700 }}>← Volver al banco</Link>
            </div>
          </div>
        </main>
      </div>
        ...
        <div className="admin-content">
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
            <div>
              ...
              <h1 style={{ fontSize: "1.5rem", fontWeight: 800, color: "#1e293b", margin: "6px 0 4px" }}>Editar pregunta</h1>
            </div>
            <button onClick={handleDelete} disabled={deleting}
              style={{ padding: "10px 20px", background: "#fef2f2", color: "#dc2626", border: "1px solid #fecaca", borderRadius: 12, fontWeight: 600, fontSize: "0.9rem", cursor: deleting ? "not-allowed" : "pointer" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2eaf7", padding: "24px 28px" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                    <div>
                      ...
                      {area === "__custom__" && <input type="text" value={areaCustom} onChange={e => setAreaCustom(e.target.value)} placeholder="Escribe el área" style={{ ...inputStyle, marginTop: 8 }} required />}
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <select value={examType} onChange={e => setExamType(e.target.value)} style={inputStyle}>
                        <option value="ICFES">ICFES</option><option value="UDEA">UDEA</option>
                      </select>
                    </div>
                    <div>
                  ...
                  <div style={{ marginBottom: 16 }}>
                    ...
                    <textarea value={enunciado} onChange={e => setEnunciado(e.target.value)} rows={4} required style={{ ...inputStyle, resize: "vertical" }} />
                  </div>
                  <div style={{ marginBottom: 16 }}>
                    <label style={labelStyle}>Contexto (opcional)</label>
                    <textarea value={contexto} onChange={e => setContexto(e.target.value)} rows={3} style={{ ...inputStyle, resize: "vertical" }} />
                  </div>
                  <div style={{ marginBottom: 16 }}>
                  ...
                  <div>
                    <label style={labelStyle}>Explicación (opcional)</label>
                    <textarea value={explicacion} onChange={e => setExplicacion(e.target.value)} rows={3} style={{ ...inputStyle, resize: "vertical" }} />
                  </div>
                </div>
                ...
                <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2eaf7", padding: "24px 28px" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
                    <div><label style={labelStyle}>Competencia</label><input type="text" value={competence} onChange={e => setCompetence(e.target.value)} style={inputStyle} /></div>
                    <div><label style={labelStyle}>Componente</label><input type="text" value={component} onChange={e => setComponent(e.target.value)} style={inputStyle} /></div>
                    <div><label style={labelStyle}>Tema</label><input type="text" value={topic} onChange={e => setTopic(e.target.value)} style={inputStyle} /></div>
                    <div><label style={labelStyle}>Subtema</label><input type="text" value={subtopic} onChange={e => setSubtopic(e.target.value)} style={inputStyle} /></div>
                    <div>
                    ...
                    <div><label style={labelStyle}>Año</label><input type="number" value={year} onChange={e => setYear(e.target.value)} min={2000} max={2030} style={inputStyle} /></div>
                  </div>
                  <div>
                  ...
                  <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2eaf7", padding: "24px 28px" }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                      {allTags.map(tag => {
                        ...
                        return <button key={tag.id} type="button" onClick={() => toggleTag(tag.id)} style={{ padding: "5px 14px", borderRadius: 20, border: `2px solid ${sel ? (tag.color ?? "#004aad") : "#e2eaf7"}`, background: sel ? (tag.color ?? "#004aad") : "#f8fafc", color: sel ? "#fff" : "#475569", fontSize: "0.8rem", fontWeight: 600, cursor: "pointer" }}>{tag.name}</button>;
                      })}
                    </div>
                  </div>
                )}
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {options.map(opt => (
                      <div key={opt.letra} style={{ display: "flex", gap: 10, alignItems: "center", padding: "8px 12px", borderRadius: 8, border: `1px solid ${opt.isCorrect ? "#004aad" : "#e2eaf7"}`, background: opt.isCorrect ? "#eff6ff" : "#fafafa" }}>
                        <div style={{ minWidth: 22, height: 22, borderRadius: "50%", background: opt.isCorrect ? "#004aad" : "#e2eaf7", color: opt.isCorrect ? "#fff" : "#64748b", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: "0.75rem", flexShrink: 0 }}>{opt.letra}</div>
                        <span style={{ fontSize: "0.83rem", color: opt.isCorrect ? "#004aad" : "#475569", fontWeight: opt.isCorrect ? 600 : 400 }}>{opt.texto || <span style={{ color: "#cbd5e1", fontStyle: "italic" }}>Opción {opt.letra}…</span>}</span>
