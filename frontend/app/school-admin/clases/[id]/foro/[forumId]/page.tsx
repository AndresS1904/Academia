...
export default function AdminForumPage() {
  ...
  async function createPost(threadId: string, content: string, parentId?: string) {
    ...
    setSavingPost(true);
    const p = await api.post<ForumPost>(`/classrooms/threads/${threadId}/posts`, { content: content.trim(), parentId }).catch(() => null);
    if (p) {
  ...
  if (loading || fetching) return (
    <div className="admin-layout"><SchoolAdminSidebar />
      <main className="admin-main"><div style={{ padding: 48, textAlign: "center", color: "#94a3b8" }}>Cargando foro…</div></main>
    </div>
  );
              ...
              <div style={{ marginBottom: 12 }}>
                {!showThreadForm ? (
                  ...
                  <div style={{ background: "#f0f4ff", borderRadius: 10, padding: 12, border: "1px solid #c7d7f0" }}>
                    <input placeholder="Título del hilo *" value={newThreadTitle} onChange={(e) => setNewThreadTitle(e.target.value)} style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1.5px solid #c7d7f0", fontSize: "0.85rem", marginBottom: 8, boxSizing: "border-box" as const }} />
                      ...
                      <div style={{ display: "flex", alignItems: "flex-start", gap: 6 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: "0.72rem", color: "#94a3b8", marginTop: 2 }}>
                            {t.author.firstName} {t.author.lastName} · {t._count.posts} respuesta{t._count.posts !== 1 ? "s" : ""} · {t.viewCount} visitas
                          </div>
                        </div>
                      </div>
                ...
                <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e2e8f0", overflow: "hidden" }}>
                  <div style={{ padding: "16px 20px", background: "#f8fafc", borderBottom: "1px solid #e2e8f0" }}>
                    ...
                    <div style={{ fontSize: "0.75rem", color: "#94a3b8", marginTop: 2 }}>
                      Por {activeThread.author.firstName} {activeThread.author.lastName} · {activeThread.viewCount} visitas
                    </div>
                  </div>
                    ...
                    <div style={{ padding: "16px 20px", maxHeight: 500, overflowY: "auto" }}>
                      {posts.length === 0 ? (
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                          {posts.map((post) => (
                            <PostCard key={post.id} post={post} onDelete={(pid) => deletePost(pid, activeThread.id)} onReply={(pid) => { setReplyTo(pid); setReplyContent(""); }} />
                          ))}
                        </div>
                      )}
                      ...
                      {replyTo && (
                        <div style={{ background: "#f0f4ff", borderRadius: 10, padding: 12, border: "1px solid #c7d7f0", marginTop: 14 }}>
                          ...
                          <textarea placeholder="Tu respuesta…" value={replyContent} onChange={(e) => setReplyContent(e.target.value)} rows={3} style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1.5px solid #c7d7f0", fontSize: "0.85rem", marginBottom: 8, boxSizing: "border-box" as const, resize: "vertical" }} />
                          <div style={{ display: "flex", gap: 8 }}>
                  ...
                  {!activeThread.isLocked && (
                    <div style={{ padding: "14px 20px", borderTop: "1px solid #e2e8f0" }}>
                      ...
                      <textarea placeholder="Escribe tu mensaje…" value={newPostContent} onChange={(e) => setNewPostContent(e.target.value)} rows={3} style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1.5px solid #e2e8f0", fontSize: "0.85rem", marginBottom: 8, boxSizing: "border-box" as const, resize: "vertical" }} />
                  ...
                  {activeThread.isLocked && (
                    <div style={{ padding: "12px 20px", borderTop: "1px solid #e2e8f0", background: "#fef2f2", color: "#dc2626", fontSize: "0.82rem", textAlign: "center", fontWeight: 600 }}>
                      🔒 Este hilo está bloqueado.
                    </div>
                  )}
                </div>
              )}
  ...
  return (
    <div style={{ background: "#f8fafc", borderRadius: 10, padding: "12px 14px", border: "1px solid #e2e8f0" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
        <div style={{ width: 32, height: 32, borderRadius: "50%", background: isAdmin ? "#fef3c7" : "#e0e7ff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: "0.78rem", color: isAdmin ? "#92400e" : "#4338ca", flexShrink: 0 }}>
          {initials}
        </div>
        <div style={{ flex: 1 }}>
          ...
          {post.isEdited && <span style={{ marginLeft: 6, fontSize: "0.68rem", color: "#94a3b8" }}>(editado)</span>}
        </div>
      ...
      {post.replies.length > 0 && (
        <div style={{ marginTop: 10, paddingLeft: 20, borderLeft: "2px solid #e2e8f0", display: "flex", flexDirection: "column", gap: 8 }}>
          {post.replies.map((r) => (
            <PostCard key={r.id} post={r} onDelete={onDelete} onReply={onReply} />
          ))}
        </div>
      )}
    </div>
