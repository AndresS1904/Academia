...
export default function EditPreguntaPage() {
  ...
  // Track whether the initial load has happened (avoid overwriting with effects)
  ...
  useEffect(() => {
    if (!user) return;
    api.get<QuestionTag[]>("/question-tags")
  ...
  useEffect(() => {
    if (!user || !id) return;
    setFetching(true);
    api.get<QuestionDetail>(`/questions/${id}`)
      .then(q => {
        setEnunciado(q.enunciado ?? "");
        setContexto(q.contexto ?? "");
        setImageUrl(q.imageUrl ?? "");
        setExplicacion(q.explicacion ?? "");
        setExamType(q.examType ?? "ICFES");
        setDifficulty(q.difficulty ?? "MEDIA");
        setSubject(q.subject ?? "");
        setQuestionType((q.questionType as any) ?? "SINGLE_CHOICE");

        if (COMMON_AREAS.includes(q.area)) {
          setArea(q.area);
          setAreaCustom("");
        } else if (q.area) {
          setArea("__custom__");
          setAreaCustom(q.area);
        }

        // Metadata
        setTopic(q.topic ?? "");
        setSubtopic(q.subtopic ?? "");
        setCompetence(q.competence ?? "");
        setComponent(q.component ?? "");
        setGradeLevel(q.gradeLevel ? String(q.gradeLevel) : "");
        setYear(q.year ? String(q.year) : "");
        setSourceType(q.sourceType ?? "PROPIA");

        // Tags
        if (q.tags) {
          setSelectedTagIds(q.tags.map((t: { tag: QuestionTag }) => t.tag.id));
        }

        // Build options from actual letters
        const loadedCount = q.options.length;
        setOptionCount(loadedCount >= 2 ? loadedCount : 4);
        const allLetras = q.options.map(o => o.letra).sort();
        setOptions(allLetras.map(letra => {
          const found = q.options.find(o => o.letra === letra);
          return found
            ? { id: found.id, letra: found.letra, texto: found.texto, imageUrl: found.imageUrl ?? "", isCorrect: found.isCorrect }
            : { letra, texto: "", imageUrl: "", isCorrect: false };
        }));

        setInitialLoaded(true);
      })
      .catch(e => alert("Error al cargar la pregunta: " + (e.message ?? "")))
      .finally(() => setFetching(false));
  }, [user, id]);

  // Only rebuild options when questionType changes AFTER initial load
  useEffect(() => {
  ...
  const previewCorrect = options.find(o => o.isCorrect);
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#f0f4ff" }}>
        ...
        <main style={{ marginLeft: 260, padding: "32px 36px" }}>
          <div style={{ padding: "56px 24px", textAlign: "center", color: "#64748b" }}>⏳ Cargando pregunta…</div>
        </main>
      </div>
    );
  }
    ...
    <div style={{ minHeight: "100vh", background: "#f0f4ff" }}>
      ...
      <main style={{ marginLeft: 260, padding: "32px 36px", minHeight: "100vh", boxSizing: "border-box" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
          <div>
            <h1 style={{ fontSize: "1.5rem", fontWeight: 800, color: "#1e293b", margin: 0 }}>Editar pregunta</h1>
            <p style={{ color: "#64748b", fontSize: "0.875rem", margin: "4px 0 0" }}>
              ID: <code style={{ fontFamily: "monospace", background: "#f1f5f9", padding: "1px 6px", borderRadius: 4 }}>{id}</code>
            </p>
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <button
              ...
              disabled={deleting}
              style={{ padding: "10px 20px", background: deleting ? "#fca5a5" : "#fef2f2", color: "#dc2626", border: "1px solid #fecaca", borderRadius: 12, fontWeight: 600, fontSize: "0.9rem", cursor: deleting ? "not-allowed" : "pointer" }}
            >
              {deleting ? "Eliminando…" : "Eliminar pregunta"}
            </button>
            <Link
              href="/admin/questions"
              style={{ padding: "10px 20px", background: "#f1f5f9", color: "#475569", borderRadius: 12, textDecoration: "none", fontWeight: 600, fontSize: "0.9rem" }}
            >
              ← Volver
            </Link>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 24, alignItems: "start" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              ...
              <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2eaf7", padding: "24px 28px" }}>
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <textarea value={enunciado} onChange={e => setEnunciado(e.target.value)} rows={4} required placeholder="Escribe el enunciado de la pregunta…" style={{ ...inputStyle, resize: "vertical" }} />
                </div>

                <div style={{ marginBottom: 16 }}>
                  <label style={labelStyle}>Contexto (opcional)</label>
                  <textarea value={contexto} onChange={e => setContexto(e.target.value)} rows={3} placeholder="Texto de contexto o lectura (opcional)…" style={{ ...inputStyle, resize: "vertical" }} />
                </div>
                ...
                <div>
                  ...
                  <textarea value={explicacion} onChange={e => setExplicacion(e.target.value)} rows={3} placeholder="Explica por qué la respuesta correcta es correcta…" style={{ ...inputStyle, resize: "vertical" }} />
                </div>
              </div>

              {/* Metadata académica */}
              <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2eaf7", padding: "24px 28px" }}>
                <h2 style={{ fontSize: "1rem", fontWeight: 700, color: "#1e293b", margin: "0 0 6px" }}>Metadata académica</h2>
                <p style={{ fontSize: "0.8rem", color: "#94a3b8", margin: "0 0 20px" }}>Usada para analítica por competencia y tema. Todos los campos son opcionales.</p>

                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
                  <div>
                    <label style={labelStyle}>Competencia</label>
                    <input type="text" value={competence} onChange={e => setCompetence(e.target.value)} placeholder="Ej: Comprensión e interpretación" style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Componente</label>
                    <input type="text" value={component} onChange={e => setComponent(e.target.value)} placeholder="Ej: Semántico, Biología…" style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Tema</label>
                    <input type="text" value={topic} onChange={e => setTopic(e.target.value)} placeholder="Ej: Álgebra lineal" style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Subtema</label>
                    <input type="text" value={subtopic} onChange={e => setSubtopic(e.target.value)} placeholder="Ej: Ecuaciones de primer grado" style={inputStyle} />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input type="number" value={year} onChange={e => setYear(e.target.value)} placeholder="Ej: 2024" min={2000} max={2030} style={inputStyle} />
                  </div>
                </div>
              ...
              {allTags.length > 0 && (
                <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2eaf7", padding: "24px 28px" }}>
                  <h2 style={{ fontSize: "1rem", fontWeight: 700, color: "#1e293b", margin: "0 0 6px" }}>Etiquetas</h2>
                  <p style={{ fontSize: "0.8rem", color: "#94a3b8", margin: "0 0 16px" }}>Selecciona las etiquetas que aplican a esta pregunta.</p>
                      ...
                      return (
                        <button
                          ...
                          style={{
                            padding: "5px 14px",
                            borderRadius: 20,
                            border: `2px solid ${sel ? (tag.color ?? "#004aad") : "#e2eaf7"}`,
                            background: sel ? (tag.color ?? "#004aad") : "#f8fafc",
              ...
              {/* Options */}
              <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2eaf7", padding: "24px 28px" }}>
                <h2 style={{ fontSize: "1rem", fontWeight: 700, color: "#1e293b", margin: "0 0 6px" }}>Opciones de respuesta</h2>
                <p style={{ fontSize: "0.8rem", color: "#94a3b8", margin: "0 0 16px" }}>
                  {questionType === "MULTIPLE_CHOICE" ? "Marca todos los checkboxes correctos." : "Selecciona el botón de la opción correcta."}
                </p>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  {options.map((opt, idx) => (
                    <div key={opt.letra} style={{
                      display: "flex", gap: 12, alignItems: "flex-start", padding: "14px 16px", borderRadius: 12,
                      border: `2px solid ${opt.isCorrect ? "#004aad" : "#e2eaf7"}`,
                      background: opt.isCorrect ? "#eff6ff" : "#fafafa", transition: "all 0.15s",
                    }}>
                      <div style={{ paddingTop: 2 }}>
                        {questionType === "MULTIPLE_CHOICE" ? (
                          <input type="checkbox" checked={opt.isCorrect} onChange={() => setCorrectOption(idx)}
                            style={{ width: 18, height: 18, cursor: "pointer", accentColor: "#004aad", marginTop: 4 }} />
                        ) : (
                          <input type="radio" name="correctOption" checked={opt.isCorrect} onChange={() => setCorrectOption(idx)}
                            style={{ width: 18, height: 18, cursor: "pointer", accentColor: "#004aad", marginTop: 4 }} />
                        )}
                      </div>
                      ...
                      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
                        <input type="text" value={opt.texto}
                          ...
                          placeholder={`Texto de la opción ${opt.letra} *`}
              ...
              <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
                <Link href="/admin/questions" style={{ padding: "12px 24px", background: "#f1f5f9", color: "#475569", borderRadius: 12, textDecoration: "none", fontWeight: 600, fontSize: "0.9rem" }}>
                  Cancelar
                </Link>
            ...
            <div style={{ position: "sticky", top: 24 }}>
              <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2eaf7", padding: "24px 28px" }}>
                <h2 style={{ fontSize: "0.95rem", fontWeight: 700, color: "#1e293b", margin: "0 0 16px" }}>Vista previa</h2>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
                  ...
                  {examType && <span style={{ padding: "3px 10px", background: "#f5f3ff", color: "#7c3aed", borderRadius: 20, fontSize: "0.74rem", fontWeight: 600 }}>{examType}</span>}
                  {difficulty && (
                    <span style={{ padding: "3px 10px", borderRadius: 20, fontSize: "0.74rem", fontWeight: 600, background: difficulty === "FACIL" ? "#dcfce7" : difficulty === "MEDIA" ? "#fef9c3" : "#fee2e2", color: difficulty === "FACIL" ? "#16a34a" : difficulty === "MEDIA" ? "#a16207" : "#dc2626" }}>
                  ...
                  {competence && <span style={{ padding: "3px 10px", background: "#fff7ed", color: "#c2410c", borderRadius: 20, fontSize: "0.74rem", fontWeight: 600 }}>{competence}</span>}
                  {topic && <span style={{ padding: "3px 10px", background: "#f0fdf4", color: "#15803d", borderRadius: 20, fontSize: "0.74rem", fontWeight: 600 }}>{topic}</span>}
                ...
                {contexto && (
                  <div style={{ background: "#f8fafc", borderRadius: 8, padding: "10px 14px", fontSize: "0.82rem", color: "#475569", lineHeight: 1.6, marginBottom: 12, borderLeft: "3px solid #004aad" }}>
                    {contexto}
                  </div>
                )}
                ...
                {imageUrl && (
                  <div style={{ marginBottom: 12 }}>
                    ...
                    <img src={imageUrl} alt="Imagen del enunciado" style={{ width: "100%", borderRadius: 8, border: "1px solid #e2eaf7" }} onError={e => { (e.target as HTMLImageElement).style.display = "none"; }} />
                  </div>
                )}
                ...
                {explicacion && (
                  <div style={{ marginTop: 14, background: "#f0fdf4", borderRadius: 8, padding: "10px 14px", fontSize: "0.8rem", color: "#166534", lineHeight: 1.6, borderLeft: "3px solid #16a34a" }}>
                ...
                {selectedTagIds.length > 0 && (
                  <div style={{ marginTop: 12, display: "flex", flexWrap: "wrap", gap: 4 }}>
                    {allTags.filter(t => selectedTagIds.includes(t.id)).map(t => (
                      <span key={t.id} style={{ padding: "2px 8px", background: t.color ?? "#e2eaf7", color: "#fff", borderRadius: 10, fontSize: "0.7rem", fontWeight: 600 }}>{t.name}</span>
                    ))}
                  </div>
                )}
