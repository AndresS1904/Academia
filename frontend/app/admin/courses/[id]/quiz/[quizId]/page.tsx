...
interface Quiz {
  id: string; title: string; description?: string; timeLimit?: number; maxAttempts: number;
  passingScore: number; status: string; questions: QuizQuestion[];
}
...
export default function AdminCourseQuizEditorPage() {
  ...
  function setCorrect(oi: number) { setOptions(prev => prev.map((o, i) => ({ ...o, isCorrect: i === oi }))); }
  ...
  const inp: React.CSSProperties = { padding: "8px 10px", borderRadius: 8, border: "1.5px solid #c7d7f0", fontSize: "0.82rem" };
  const btnP: React.CSSProperties = { padding: "8px 18px", background: "#004aad", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 700, fontSize: "0.82rem" };
  const btnS: React.CSSProperties = { padding: "8px 12px", background: "#f1f5f9", border: "none", borderRadius: 8, cursor: "pointer", color: "#64748b", fontSize: "0.82rem" };
      ...
      <main style={{ marginLeft: 260, padding: "28px 36px", minHeight: "100vh", boxSizing: "border-box" }}>
        <div style={{ maxWidth: 860 }}>
          <div style={{ marginBottom: 16 }}>
            <Link href={`/admin/courses/${courseId}`} style={{ fontSize: "0.875rem", color: "#64748b", textDecoration: "none" }}>← Volver al curso</Link>
          </div>
            ...
            {!editMeta ? (
              <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                    <h1 style={{ fontWeight: 800, fontSize: "1.2rem", color: "#1e293b", margin: 0 }}>🧪 {quiz.title}</h1>
                  ...
                  <div style={{ fontSize: "0.72rem", color: "#94a3b8" }}>{quiz.timeLimit ? `${quiz.timeLimit} min` : "Sin límite"} · {quiz.maxAttempts} intento(s) · Aprobación: {quiz.passingScore}%</div>
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  {quiz.status !== "PUBLISHED" && <button onClick={publishQuiz} style={{ padding: "7px 14px", background: "#16a34a", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 700, fontSize: "0.78rem" }}>Publicar</button>}
              ...
              <div>
                <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr", gap: 10, marginBottom: 10 }}>
                  <input value={metaTitle} onChange={e => setMetaTitle(e.target.value)} placeholder="Título" style={inp} />
                  <input value={metaTime} onChange={e => setMetaTime(e.target.value)} placeholder="Tiempo (min)" type="number" style={inp} />
                  <input value={metaMax} onChange={e => setMetaMax(e.target.value)} placeholder="Intentos" type="number" style={inp} />
                  <input value={metaPass} onChange={e => setMetaPass(e.target.value)} placeholder="% Aprobación" type="number" style={inp} />
                  <select value={metaStatus} onChange={e => setMetaStatus(e.target.value)} style={inp}>
                    <option value="DRAFT">DRAFT</option><option value="PUBLISHED">PUBLISHED</option><option value="ARCHIVED">ARCHIVED</option>
                  </select>
                </div>
                <textarea value={metaDesc} onChange={e => setMetaDesc(e.target.value)} placeholder="Descripción" rows={2} style={{ ...inp, width: "100%", marginBottom: 10, boxSizing: "border-box", resize: "vertical" }} />
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={saveMeta} disabled={saving} style={btnP}>{saving ? "…" : "Guardar"}</button>
                  <button onClick={() => setEditMeta(false)} style={btnS}>Cancelar</button>
                </div>
              </div>
            )}
          </div>
          ...
          {showQForm && (
            <div style={{ background: "#f0f4ff", borderRadius: 12, padding: 18, border: "1px solid #c7d7f0", marginBottom: 16 }}>
              <textarea placeholder="Enunciado *" value={qContent} onChange={e => setQContent(e.target.value)} rows={3} style={{ ...inp, width: "100%", marginBottom: 10, boxSizing: "border-box", resize: "vertical" }} />
              <div style={{ marginBottom: 12 }}>
                ...
                {options.map((o, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                    ...
                    <input placeholder={`Opción ${i + 1}`} value={o.content}
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 10 }}>
                <input placeholder="Puntos" type="number" step="0.5" value={qPoints} onChange={e => setQPoints(e.target.value)} style={{ ...inp, width: 90 }} />
                <textarea placeholder="Explicación (opcional)" value={qExplanation} onChange={e => setQExplanation(e.target.value)} rows={2} style={{ ...inp, flex: 1, resize: "vertical" }} />
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={saveQuestion} disabled={savingQ} style={btnP}>{savingQ ? "…" : "Agregar pregunta"}</button>
                <button onClick={() => setShowQForm(false)} style={btnS}>Cancelar</button>
              </div>
            </div>
          )}
                ...
                <div key={q.id} style={{ background: "#fff", borderRadius: 12, border: "1px solid #e2e8f0", padding: "14px 16px" }}>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
                      ...
                      <button onClick={() => deleteQuestion(q.id)} style={{ padding: "4px 10px", border: "1px solid #fee2e2", borderRadius: 6, background: "#fff", cursor: "pointer", fontSize: "0.72rem", color: "#dc2626" }}>Eliminar</button>
                    </div>
                  </div>
                </div>
              ))}
