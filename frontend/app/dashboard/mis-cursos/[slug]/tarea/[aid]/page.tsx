...
interface Activity { id: string; title: string; description?: string; instructions?: string; dueDate?: string; maxScore: number }
interface Submission { id: string; status: string; content?: string; submittedAt?: string; score?: number; feedback?: string; gradedAt?: string }
...
export default function StudentCourseTaskPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.get<Activity>(`/classrooms/topic-activities/${activityId}/student`).catch(() => null),
      api.get<Submission>(`/classrooms/topic-activities/${activityId}/my-submission`).catch(() => null),
    ]).then(([act, sub]) => {
  ...
  async function submit() {
    ...
    setSubmitting(true);
    const res = await api.post<Submission>(`/classrooms/topic-activities/${activityId}/submit`, { content: content.trim() }).catch(() => null);
    if (res) setSubmission(res);
    setSubmitting(false);
  }

  if (loading || fetching) return (
    <><Navbar /><div className="dashboard-layout"><DashboardSidebar />
      ...
      <main className="dashboard-main">
        <div className="dashboard-content" style={{ maxWidth: 700 }}>
          ...
          <div style={{ marginBottom: 24 }}>
            <Link href={`/dashboard/mis-cursos/${slug}`} style={{ fontSize: "0.85rem", color: "#64748b", textDecoration: "none" }}>← Volver al curso</Link>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
              ...
              {statusInfo && (
                <span style={{ padding: "2px 10px", background: statusInfo.bg, color: statusInfo.color, borderRadius: 20, fontSize: "0.72rem", fontWeight: 700 }}>{statusInfo.label}</span>
              )}
            </div>
          </div>
          ...
          <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e2e8f0", padding: "18px 20px" }}>
            <div style={{ fontWeight: 700, fontSize: "0.88rem", color: "#1e293b", marginBottom: 12 }}>{submission ? "Tu entrega" : "Entregar tarea"}</div>
            ...
            <textarea value={content} onChange={e => setContent(e.target.value)} disabled={!canEdit}
              placeholder="Escribe tu respuesta aquí…" rows={8}
              style={{ width: "100%", padding: "12px 14px", borderRadius: 10, border: `1.5px solid ${!canEdit ? "#e2e8f0" : "#c7d7f0"}`, fontSize: "0.9rem", lineHeight: 1.7, resize: "vertical", boxSizing: "border-box", background: !canEdit ? "#f8fafc" : "#fff", color: "#374151", marginBottom: 14 }} />
            {canEdit && (
            ...
            {!canEdit && submission?.status !== "GRADED" && (
              <div style={{ fontSize: "0.82rem", color: "#64748b" }}>Tu entrega está siendo revisada.</div>
            )}
          </div>
