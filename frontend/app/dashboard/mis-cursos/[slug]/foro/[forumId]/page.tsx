...
function PostCard({ post, userId, onDelete, onReply }: { post: ForumPost; userId?: string; onDelete: (id: string) => void; onReply: (id: string) => void }) {
  ...
  return (
    <div style={{ background: "#f8fafc", borderRadius: 10, padding: "12px 14px", border: "1px solid #e2e8f0" }}>
      ...
      <div style={{ display: "flex", gap: 6, marginTop: 10 }}>
        ...
        {isOwn && <button onClick={() => onDelete(post.id)} style={{ padding: "3px 10px", border: "1px solid #fee2e2", borderRadius: 6, background: "#fff", cursor: "pointer", fontSize: "0.72rem", color: "#dc2626" }}>Eliminar</button>}
      </div>
      {post.replies.length > 0 && (
        <div style={{ marginTop: 10, paddingLeft: 20, borderLeft: "2px solid #e2e8f0", display: "flex", flexDirection: "column", gap: 8 }}>
          {post.replies.map(r => <PostCard key={r.id} post={r} userId={userId} onDelete={onDelete} onReply={onReply} />)}
        </div>
      )}
    </div>
  );
          ...
          <div className="dashboard-content" style={{ maxWidth: 900 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: 20, alignItems: "start" }}>
              <div>
                <div style={{ marginBottom: 10 }}>
                  {!showThreadForm ? (
                    ...
                    <div style={{ background: "#f0f4ff", borderRadius: 10, padding: 12, border: "1px solid #c7d7f0" }}>
                      <input placeholder="Título *" value={newThreadTitle} onChange={(e) => setNewThreadTitle(e.target.value)}
                        style={{ width: "100%", padding: "8px 10px", borderRadius: 8, border: "1.5px solid #c7d7f0", fontSize: "0.82rem", marginBottom: 8, boxSizing: "border-box" as const }} />
                      <div style={{ display: "flex", gap: 6 }}>
                        <button onClick={createThread} disabled={savingThread}
                          style={{ flex: 1, padding: "7px 0", background: "#004aad", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 700, fontSize: "0.78rem" }}>
                          {savingThread ? "…" : "Crear"}
                        </button>
                        <button onClick={() => { setShowThreadForm(false); setNewThreadTitle(""); }}
                          style={{ padding: "7px 10px", background: "#f1f5f9", border: "none", borderRadius: 8, cursor: "pointer", color: "#64748b" }}>✕</button>
                      </div>
                    </div>
                  )}
                </div>
                ...
                {threads.length === 0 ? (
                  <div style={{ textAlign: "center", padding: "32px 0", color: "#94a3b8", fontSize: "0.85rem" }}>Sin hilos aún. ¡Crea el primero!</div>
                ) : (
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {threads.map(t => (
                      <div key={t.id} onClick={() => openThread(t)}
                        style={{ background: activeThread?.id === t.id ? "#f0f4ff" : "#fff", borderRadius: 10, border: activeThread?.id === t.id ? "2px solid #004aad" : "1px solid #e2e8f0", padding: "11px 13px", cursor: "pointer" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          {t.isPinned && <span style={{ fontSize: "0.7rem" }}>📌</span>}
                          {t.isLocked && <span style={{ fontSize: "0.7rem" }}>🔒</span>}
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: "0.7rem", color: "#94a3b8", marginTop: 2 }}>{t.author.firstName} {t.author.lastName} · {t._count.posts} respuesta(s)</div>
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              <div>
                {!activeThread ? (
                  <div style={{ textAlign: "center", padding: "60px 24px", background: "#f8fafc", borderRadius: 14, border: "2px dashed #e2e8f0", color: "#94a3b8" }}>
                    ...
                    <div style={{ fontWeight: 600 }}>Selecciona un hilo para participar</div>
                  </div>
                ) : (
                  <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e2e8f0", overflow: "hidden" }}>
                    ...
                    {loadingPosts ? (
                      ...
                      <div style={{ padding: "16px 18px", maxHeight: 450, overflowY: "auto" }}>
                        {posts.length === 0 ? (
                          <div style={{ color: "#94a3b8", fontSize: "0.85rem", textAlign: "center", padding: "24px 0" }}>Sin respuestas aún. ¡Sé el primero!</div>
                        ) : (
                          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                            {posts.map(post => <PostCard key={post.id} post={post} userId={user?.id} onDelete={pid => deleteMyPost(pid, activeThread.id)} onReply={pid => { setReplyTo(pid); setReplyContent(""); }} />)}
                          </div>
                        )}
                        {replyTo && (
                          <div style={{ background: "#f0f4ff", borderRadius: 10, padding: 12, border: "1px solid #c7d7f0", marginTop: 14 }}>
                            ...
                            <textarea placeholder="Tu respuesta…" value={replyContent} onChange={e => setReplyContent(e.target.value)} rows={3}
                              style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1.5px solid #c7d7f0", fontSize: "0.85rem", marginBottom: 8, boxSizing: "border-box", resize: "vertical" }} />
                            <div style={{ display: "flex", gap: 8 }}>
                              <button onClick={() => createPost(activeThread.id, replyContent, replyTo)} disabled={savingPost}
                                style={{ padding: "8px 16px", background: "#004aad", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 700, fontSize: "0.82rem" }}>{savingPost ? "…" : "Responder"}</button>
                              <button onClick={() => { setReplyTo(null); setReplyContent(""); }}
                                style={{ padding: "8px 12px", background: "#f1f5f9", border: "none", borderRadius: 8, cursor: "pointer", color: "#64748b" }}>Cancelar</button>
                            </div>
                          </div>
                        )}
                      </div>
                    ...
                    {!activeThread.isLocked ? (
                      <div style={{ padding: "14px 18px", borderTop: "1px solid #e2e8f0" }}>
                        ...
                        <textarea placeholder="Escribe tu mensaje…" value={newPostContent} onChange={e => setNewPostContent(e.target.value)} rows={3}
                          style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1.5px solid #e2e8f0", fontSize: "0.85rem", marginBottom: 8, boxSizing: "border-box", resize: "vertical" }} />
                        <button onClick={() => createPost(activeThread.id, newPostContent)} disabled={savingPost}
                          style={{ padding: "9px 20px", background: "#004aad", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 700, fontSize: "0.85rem" }}>{savingPost ? "Enviando…" : "Publicar mensaje"}</button>
                      </div>
                    ) : (
                      <div style={{ padding: "12px 18px", borderTop: "1px solid #e2e8f0", background: "#fef2f2", color: "#dc2626", fontSize: "0.82rem", textAlign: "center", fontWeight: 600 }}>🔒 Este hilo está bloqueado.</div>
                    )}
                  </div>
                )}
              </div>
